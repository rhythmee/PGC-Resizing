...
file_path = 'nodes_voltage_16x.csv'
#export variables
...
for i in range(722): #index of PGC
    ...
    modified_resistance = size_resistance[3]
    ...
    subprocess.run([python_executable, f'{ROOT_DIR}/src/app_16x.py', f'--config={config_file_name}'])

    matrix = extract_values_from_csv(file_path)
    
    output_file = f"{ROOT_DIR}/data/pgc_{i}_size_16x.npy" 
