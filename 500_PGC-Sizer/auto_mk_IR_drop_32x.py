...
config_file_name = f'{ROOT_DIR}/config/IS_cur_10x_size_2x.yaml'
file_path = 'nodes_voltage_32x.csv'
#export variables
...
for i in range(722): #index of PGC
    #for j in list_pgc_name:
    pgc_x_coord = pgc_location[i][0]
    pgc_y_coord = pgc_location[i][1]
    modified_resistance = size_resistance[4]
    ...
    subprocess.run([python_executable, f'{ROOT_DIR}/src/app_32x.py', f'--config={config_file_name}'])
    matrix = extract_values_from_csv(file_path)
    
    #output_file = f"{ROOT_DIR}/data/pgc_{i}_size_{j}.npy" 
    output_file = f"{ROOT_DIR}/data/pgc_{i}_size_32x.npy" 
